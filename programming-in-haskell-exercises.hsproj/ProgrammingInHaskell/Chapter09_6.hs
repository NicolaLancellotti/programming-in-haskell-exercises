...
valid' Exp x y = x ^ y > 0 -- overflows are more frequent with exponentiation
...
instance Ord Expr where
  compare lhs rhs 
    | expensiveOpLhs < expensiveOpRhs = LT
    | expensiveOpLhs > expensiveOpRhs = GT
    | otherwise = EQ
    where
    expensiveOpLhs = numOp Mul lhs + numOp Div lhs  + numOp Exp lhs
    expensiveOpRhs = numOp Mul rhs + numOp Div rhs  + numOp Exp rhs
  

numOp :: Op -> Expr -> Int
numOp _ (Val _)      = 0
numOp op (App o l r) = numOp op l + numOp op r + if op == o then 1 else 0